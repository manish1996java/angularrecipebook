...
export class AuthService {
  ...
  signUp(user:User){
      this.http.post<{message:String}>('http://localhost:8888/user/signup',user).subscribe((res)=>{
        console.log(res);
      })
  }

  login(user:User){
    this.http.post<{message:string,token:string,expiresin:number}>('http://localhost:8888/user/login',user).subscribe((res)=>{
